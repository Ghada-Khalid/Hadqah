...
async function initAssessment() {
  try {
    const params   = new URLSearchParams(location.search);
     const sidParam = params.get('sessionId');

    // grab sessionId if passed in URL
    window.sessionId = params.has('sessionId')
      ? parseInt(params.get('sessionId'), 10)
      : null;
...
// 2) Build the control rows & count totalControls
...
// 3) Row HTML with data-control-id on the dropdown
function createControlRow(mainDomain, subdomain, type, ref, clause) {
  ...
  row.innerHTML = `
    ...
    <div class="control-cell"><div class="control-description">${clause}</div></div>
    ...
    <div class="control-cell">
      <textarea class="textarea-field corrective-textarea" placeholder="Corrective…"></textarea>
...
// 4) Real-time progress bar/text
function updateProgress() {
  const completed = Array.from(document.querySelectorAll('.status-dropdown'))
    .filter(sel => sel.value !== '').length;
  const pct = totalControls
    ? Math.round((completed / totalControls) * 100)
    : 0;

  document.getElementById('progress-text').textContent =
    `${completed} of ${totalControls} controls completed (${pct}%)`;
  document.getElementById('progress-bar').style.width = `${pct}%`;
}

// 5) Save button label + enable/disable
...
// 6) Attach change listeners to both updateProgress and updateSaveButton
...
function collectAssessmentPayload() {
  return Array.from(document.querySelectorAll('.status-dropdown'))
    ...
    .map(sel => {
      ...
      return {
        controlId:             parseInt(sel.dataset.controlId, 10),
        sessionId:             window.sessionId,
        status:                sel.value,
        // Now these will find the textareas you just renamed:
        correctiveProcedures:  row.querySelector('.corrective-textarea').value.trim(),
        remarks:               row.querySelector('.remarks-textarea').value.trim(),
        // If you have compliance-level:
        complianceLevel:       (row.querySelector('.compliance-level-input')?.value.trim() || ''),
        expectedComplianceDate: row.querySelector('.date-input').value || null,
...
async function saveAssessment() {
  ...
  let resp;
  try {
    resp = await fetch('/api/assessments/batch', {
      method:  'POST',
      headers: { 'Content-Type':'application/json' },
      body:    JSON.stringify({ assessments: collectAssessmentPayload() })
    });
  } catch (networkError) {
    console.error('Network error:', networkError);
    showError('Network error: ' + networkError.message);
    btn.textContent = `Save ${completed} Assessments`;
    btn.disabled   = false;
    return;
  }

  // If we get a non-2xx status:
  if (!resp.ok) {
    ...
    console.error(`Save failed [${resp.status}]:`, errorMsg);
  ...
  alert(`✅ Saved ${data.count} assessments. Redirecting…`);
